...
## Import metadata
```{r, warning = FALSE}
path.p <- "data/TPC/PI_Curves" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/TPC/PI_Curves/pi_curves_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/TPC/PI_Curves/pi_curves_run_metadata.csv")
...
metadata <- metadata %>%
  select(Species, Stage, colony_id, Run, Temp.Cat, Chamber.Vol.L, Date, Start.time, Stop.time, Light_Value, Light_Level)
```
...
#select only the essential columns
mx <- select(metadata, Species, Stage, colony_id, Run, Chamber.Vol.L, Temp.Cat)
nrow(mx)
nrow(distinct(df))
...
pr.out <- pr %>% 
  select(Species, Stage, colony_id, Light_Value.x, Temp.Cat.x, Run, micromol.cm2.s, micromol.cm2.h)
length(unique(pr.out$colony_id))

# Write to output file
write.csv(pr.out, "output/TPC_pi_curve_extracted_rates.csv")
```
#plot  data
```{r}
pr.out$PAR <- as.numeric(pr.out$Light_Value.x) 
pr.out$Pc <- as.numeric(pr.out$micromol.cm2.s) 

pr.out %>% 
ggplot(aes(x=PAR, y=Pc, color=Stage, group=colony_id))+ 
  geom_point()+
  geom_line()+
