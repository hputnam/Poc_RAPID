...
adult.meandrina.CI.plot <- ggplot() +
  geom_line(aes(temp, .fitted), adult.meandrina.preds, col = 'orange') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), adult.meandrina.boot1_conf_preds, fill = 'orange', alpha = 0.3) +
  geom_point(aes(temp, rate), d.meandrina.adult, size = 2, alpha = 0.5,col = 'orange') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
...
recruit.meandrina.CI.plot <- ggplot() +
  geom_line(aes(temp, .fitted), recruit.meandrina.preds, col = 'red') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), recruit.meandrina.boot1_conf_preds, fill = 'red', alpha = 0.3) +
  geom_point(aes(temp, rate), d.meandrina.recruit, size = 2, alpha = 0.5,col = 'red') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Rate (µmol O2/cm2/h)')
...
```{r}
 #set plot colors
cols <- c("Adult"="orange",  "Recruit"="red")
...
PM.TPC.plot <- ggplot(data=d, aes(x=temp)) +
   ...
   geom_line(aes(temp, .fitted), adult.meandrina.preds, col = 'orange', size=2) +
   geom_line(aes(temp, .fitted), recruit.meandrina.preds, col = "red", size=2) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), adult.meandrina.boot1_conf_preds, fill = "orange", alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), recruit.meandrina.boot1_conf_preds, fill = "red", alpha = 0.3) +
   xlim(16,41)+
  #ylim(0,0.45)+
...
estimate.plots <- ggplot(All_params, aes(Stage, estimate, color=Stage)) +
  geom_point(size = 2) +
  scale_color_manual(name="Stage", values=c("orange", "red"))+
  geom_linerange(aes(ymin = conf_lower, ymax = conf_upper)) +
  theme_bw() +
...
ggsave("output/Pmeandrina_Pgross_TPC_and_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
ggsave("output/Pmeandrina_Pgross_TPC_and_estimates.jpg", figure, dpi=300, w=6, h=8, units="in")
...
#d <- d[,-c(1,5,6,7,8,9,11)]
#str(d)

colnames(d)
#colnames(d) <- c("Species", "colony_id", "temp","Stage", "rate")
...
figure <- ggarrange(PM.TPC.plot , estimate.plots,
                    hjust = c(.01,.01),
                    vjust = c(1.2,.01),
                    labels = c("A) Rdark", "B) BootStrap TPC Estimates"),
                    ncol = 1, nrow = 2,
                    heights=c(1,0.5))
figure
...
data3$group <- paste0(data3$colony_id, data3$Temperature)

data3$check <- paste0(data3$colony_id, data3$Temperature, data3$Light)
...
data4 <- left_join(data.light3, data.dark3, by = "group") %>%
  select(Species = Species.x, 
         colony_id = colony_id.x, 
         temp = avg_temp_interval.x, 
         temp_dark = avg_temp_interval.y, 
...
data4$Pgross <- data4$Pnet- data4$Rdark

data4$Rdark <- -data4$Rdark
data4$Rdark <- replace(data4$Rdark, data4$Rdark<0,0)
...
colnames(d.grandis.adult) <-c("Species", "colony_id", "temp", "temp_dark", "Stage", "Pnet",
                                "Rdark",  "rate")
...
colnames(d.meandrina.adult) <-c("Species", "colony_id", "temp", "temp_dark", "Stage", "Pnet",
                                "Rdark",  "rate")
...
colnames(d.tuahiniensis.adult) <-c("Species", "colony_id", "temp", "temp_dark", "Stage", "Pnet",
                                "Rdark",  "rate")
...
colnames(d.verrucosa.adult) <-c("Species", "colony_id", "temp", "temp_dark", "Stage", "Pnet",
                                "Rdark",  "rate")
...
ggsave("output/AllSpecies_Pgross_TPC_and_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
ggsave("output/AllSpecies_Pgross_TPC_and_estimates.jpg", figure, dpi=300, w=6, h=8, units="in")
...
colnames(d.grandis.adult) <-c("Species", "colony_id", "temp_light", "temp", "Stage", "Pnet",
                                "rate",  "Pgross")
...
colnames(d.meandrina.adult) <-c("Species", "colony_id", "temp_light", "temp", "Stage", "Pnet",
                                "rate",  "Pgross")
...
colnames(d.tuahiniensis.adult) <-c("Species", "colony_id", "temp_light", "temp", "Stage", "Pnet",
                                "rate",  "Pgross")
...
colnames(d.verrucosa.adult) <-c("Species", "colony_id", "temp_light", "temp", "Stage", "Pnet",
                                "rate",  "Pgross")
