...
#load wax data
wax.data <- read.csv("data/TPC/20241123_TPC_surface_area_data.csv", header=TRUE)
...
boxplot(smpls$surface.area.cm2)
hist(smpls$surface.area.cm2)
range(stnds$surface.area.cm2)

smpls %>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("output/20241123_TPC_surface_area_data_calculated.csv")
