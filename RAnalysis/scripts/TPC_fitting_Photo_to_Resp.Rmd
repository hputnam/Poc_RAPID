...
str(data)
data$Temperature.cat <- data$Temperature
...
data$group <- paste0(data$colony_id.x, "-", data$Temperature.cat)
...
d <- data %>%
  ...
  filter(!group == "POT-024-28")%>%
  filter(!group ==  "POG-150-32") %>%
  filter(! curve_id == "26")

d <- d[,-c(1,4,7,9)]
str(d)

colnames(d) <- c("Species", "colony_id", "temp","Light", "rate", "Temperature.cat", "curve_id", "group" )
...
d <- left_join(Photo, Resp, by="group")

d$Pgross<- d$rate.x-d$rate.y

d <- d %>%
  subset(select = c("Species.x","colony_id.x", "curve_id.x", "temp.x","Temperature.cat.x", "rate.x", "rate.y", "Pgross"))

colnames(d) <- c("Species","colony_id", "curve_id","temp","Temperature.cat", "rate.Pnet", "rate.Rd", "rate.Pgross")

d$PR.ratio <- d$rate.Pgross/abs(d$rate.Rd) 

d$rate <- d$rate.Pgross

d$rate <- replace(d$rate, d$rate<0,0)
...
d %>% 
ggplot(aes(x=temp, y=PR.ratio, color=Species, group=colony_id))+ 
  geom_point()+
  geom_line()+
  ylab("P:R")+
  facet_wrap(~ Species,  ncol = 5)+
