...
d$micromol.cm2.h <- -d$micromol.cm2.h
d$micromol.cm2.h <- replace(d$micromol.cm2.h, d$micromol.cm2.h<0,0)
...
figure <- ggarrange(TPC.plot , estimate.plots, hjust = c(.01,.01),
                    vjust = c(1.2,.01),
                    labels = c("A) LEDR", "B) BootStrap TPC Estimates"),
                    ncol = 1, nrow = 2,
                    heights=c(1,0.5))
figure

ggsave("output/Respiration_TPC_and_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
ggsave("output/Respiration_TPC_and_estimates.jpg", figure, dpi=300, w=6, h=8, units="in")
