...
#filtering expression values for selected data set and for PoverA
```{r}
...
# count unannotated genes in fiiltered list
...
length(unique(reducedTerms.EMB$parentTerm))
#list the unique parent terms
unique(reducedTerms.EMB$parentTerm)

#plot GO terms under their parent terms
pdf("figures/BP_embryo_expression.pdf", width = 16, height = 8)
...
reducedTerms.EMB <- left_join(reducedTerms.EMB, EMB.BP.INT, by="category")


#identify the links between terms
edges <- reducedTerms.EMB[,c(8,7)]
vertices <- reducedTerms.EMB[,c(7,6,8)]
#generate a graph object
mygraph <- graph_from_data_frame(edges, vertices=vertices)

pdf("figures/circle_BP_embryo_expression.pdf", width = 16, height = 8)
ggraph(mygraph, layout = 'circlepack') + 
  geom_node_circle(aes(fill = parentTerm)) +
  theme_void()
dev.off()

pdf("figures/2circle_BP_embryo_expression.pdf", width = 16, height = 8)
ggraph(mygraph, layout = 'partition', circular = TRUE) +
  geom_node_arc_bar(aes(fill = parentTerm)) +
  coord_fixed()
dev.off()


pdf("figures/square_BP_embryo_expression.pdf", width = 16, height = 8)
ggraph(mygraph, layout = 'treemap') + 
  geom_node_tile(aes(fill = parentTerm))
  #geom_tile_text(aes(label = parentTerm, size = 3)) 
dev.off()
