...
Remove samples that have inaccurate slope extraction for respiration rates (determined by PDF output files and outlier plots)  
```{r}
oxygen<-oxygen%>%
  mutate(code=paste(Plate, Chamber.ID))%>%
  filter(!c(Plate==2 & Chamber.ID=="D5")) #%>%
  #filter(!code=="3 D4")%>%
  #filter(!code=="7 D6")%>%
  #filter(!code=="8 B4")%>%
  #filter(!code=="12 A3")%>%
...
oxygen %>% 
  filter(Type=="Sample") %>%
  ggplot(aes(Phenotype, P.umol.min))+
  geom_point()+
  geom_hline(yintercept=0, linetype="dashed", color = "black")+
  facet_wrap(vars(Light))+
  theme_bw()
...
mean.blank.data <- blank_data %>% group_by(Phenotype,Light) %>%
summarise(mean = mean(P.umol.min, na.rm=TRUE)) #mean P phase blanks

mean.blank.data %>% 
  ggplot(aes(Phenotype, mean))+
  geom_point()+
  geom_hline(yintercept=0, linetype="dashed", color = "black")+
  facet_wrap(vars(Light))+
  theme_bw()
...
oxygen %>% 
  ggplot(aes(Phenotype, P.umol.min.corr))+
  geom_point()+
  geom_hline(yintercept=0, linetype="dashed", color = "black")+
  facet_wrap(vars(Light))+
  theme_bw()
...
oxygen.bio %>% group_by(Light, Phenotype) %>%
  summarize(max = max(P.nmol.org.min))

oxygen.bio %>% group_by(Light, Phenotype) %>%
  summarize(min = min(P.nmol.org.min))

# Remove outliers
oxygen.bio<-oxygen.bio%>%
  mutate(code=paste(Plate, Chamber.ID))%>%
  filter(!c(Plate==2 & Chamber.ID=="A1")) 
...
2-way ANOVA
```{r}
mod1 <- lm(P.nmol.org.min ~as.factor(Phenotype)*as.factor(Light), data=oxygen.bio )
summary(mod1)
...
oxygen.bio.mean <- oxygen.bio %>% 
        group_by(Phenotype, Light) %>% 
        summarise(y= mean(P.nmol.org.min))

supp.labs <- c("Photosynthesis (Pnet)", "Respiration (LEDR)")
names(supp.labs) <- c("1", "2")

ggplot(oxygen.bio.mean, aes(x = Phenotype, y = y, shape = Phenotype)) +
  geom_point(data = oxygen.bio, aes(Phenotype, P.nmol.org.min, group = Phenotype)) +
  geom_point(data = oxygen.bio.mean, size = 4, shape = 3, color="black")+
  geom_hline(yintercept=0, linetype="dashed", color = "black")+
