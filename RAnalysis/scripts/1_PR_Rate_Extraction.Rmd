...
sample.info <- read_csv(file = "data/TPC/PR/PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/TPC/PR/PR_run_metadata.csv")
...
# #NEED TO ADD IN CODE TO EXTRACT MEAN TEMPERATURE OF EACH LEVEL
...
pr.out <- pr %>% 
  select(Species, colony_id.x, Light_Level.x, Run, micromol.cm2.s, micromol.cm2.h)
...
pr.out <- pr.out %>%
filter(Temperature >20 & Light == 0)#%>%
#filter(Temperature <39)

pr.out %>% 
ggplot(aes(x=Temperature, y=-Pc, color=Species, group=colony_id.x))+ 
  geom_point()+
  geom_line()+
