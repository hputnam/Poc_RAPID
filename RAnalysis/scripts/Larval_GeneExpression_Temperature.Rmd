...
pdf("figures/L28_GO_Enrichment_semsim.pdf", width = 7, height = 6)
L28BPGO <- d1 %>% 
  ggplot(aes(x=category, y=-log10(over_represented_pvalue), colour=parentTerm)) +
  #ylim(-16,16)+
  geom_segment(aes(x=category, xend=category, y=0, yend=-log10(over_represented_pvalue), colour = parentTerm),
...
pdf("figures/Temperature_DEG_GO.pdf", height=18, width = 20)
(DEG_LFC)|(L31BPGO / L28BPGO) + plot_layout(heights = c(2, 1), guides = "collect")
dev.off()
