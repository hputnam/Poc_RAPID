...
## Import metadata
```{r, warning = FALSE}
path.p <- "data/TPC/PR_Nov" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/TPC/PR_Nov/PR_sample_metadata.csv")
str(sample.info)

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/TPC/PR_Nov/PR_run_metadata.csv")
str(run.info)
...
```{r, warning = FALSE}
#Use start time of each step from the metadata to separate data by  stop
df <- df %>%
...
df <- df %>%
  mutate(data_plot = map2(data, colony_id, ~ ggplot(.x, aes(x = Time, y = Value)) + 
                            facet_wrap(~ Light_Level, scales = "free") +
                            geom_point() +
                            labs(title = .y)),
...
sa <- read.csv("output/20241123_TPC_surface_area_data_calculated.csv")
...
write.csv(pr.out, "output/Nov_pr_curve_extracted_rates.csv")
...
pr.out %>% 
  ...
  filter(Light==560)%>%
ggplot(aes(x=avg_temp_interval, y=Pc, color=colony_id, group=colony_id))+ 
  geom_point()+
  geom_line()+
  ylab("µmol O2 cm^2 s-1")+
  facet_wrap(~ colony_id,  ncol = 5)+
